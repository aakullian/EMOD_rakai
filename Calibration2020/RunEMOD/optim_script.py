...
# BASE_POPULATION_SCALE_FACTOR = 0.05  # ORIGINAL PRODUCTION CALIBRATION VALUE

N_ITERATIONS = 2
N_SAMPLES_PER_ITERATION = 12  # the number of distinct parameter sets to run per iteration
N_REPLICATES = 2  # replicates > 1 helps OptimTool to be more stable at the cost of more simulations. 3 is recommended.
...
ingest_xlsm_filename = os.path.join('data', 'calibration_ingest_form.xlsm')
...
static_files_dir = os.path.join(dir_path, 'input_files', 'static')
...
for filename in demographics_filenames:
    print('loading demographics file: %s' % filename)
